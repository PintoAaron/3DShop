...
class KafkaStreamProducer:
    ...
    def __connect_to_kafka(cls):
        try:
            ...
            if not producer.bootstrap_connected():
                logger.error(
                    f"{KafkaStreamProducer.__connect_to_kafka.__name__} - Unable to connect to kafka server")

            return producer

        except errors.NoBrokersAvailable:
            logger.error(
                f"{KafkaStreamProducer.__connect_to_kafka.__name__} - No Kafka brokers available")
            raise
    ...
    def send_data_to_topic(cls, topic: str, message: list):
        ...
        try:
            ...
            logger.error(
                f"{KafkaConsumer.send_data_to_topic.__name__} - Timeout error while sending data to kafka server")
        except errors.KafkaError as e:
            logger.error(
                f"{KafkaConsumer.send_data_to_topic.__name__} - Error while sending data to kafka server")
            logger.exception(e)
        finally:
            producer.close()
